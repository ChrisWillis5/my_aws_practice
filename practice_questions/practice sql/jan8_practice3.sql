...
INSERT INTO cards_ingest.tran_fact
	(tran_id, cust_id, state_id, tran_ammt, tran_date) VALUES
...
(cust_id,state_id,zip_cd , cust_first_name, cust_last_name, start_date,end_date,active_flag)
VALUES
...
ALTER TABLE cards_ingest.cust_dim_details
RENAME state_cd TO state_id
...
SELECT cust_id, state_id, tran_ammt, 
SUM(tran_ammt) OVER(ORDER BY tran_ammt ROWS BETWEEN 
    UNBOUNDED PRECEDING AND CURRENT ROW) as tran_ammt_running_total
...
SELECT *, 
count(tran_ammt) over() as num_tran_edit
FROM (
    SELECT cust_id, state_id, tran_ammt,
    count(tran_ammt) over() as num_tran,
    count(tran_ammt) over(partition by state_id) as num_tran_per_state,
